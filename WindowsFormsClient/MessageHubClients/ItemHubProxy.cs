...
namespace WinFormsClient.MessageHubClients
{
    ...
    public class ItemHubProxy : HubProxyInvoker
    {
        ...
        {
            //修改商品库存消息
            HubProxy.On<object>("ItemStockChanged", (x) => { base.InvokeEvent(ItemStockChanged, x); });

            //商品卖空消息
            HubProxy.On<object>("ItemZeroStock", x => base.InvokeEvent(ItemStockChanged, x));

            //商品更新消息
            HubProxy.On<object>("ItemUpdate", x => InvokeEvent(ItemUpdate, x));

            //商品删除消息
            HubProxy.On<object>("ItemDelete", x => InvokeEvent(ItemDelete, x));

            //商品下架消息
            HubProxy.On<object>("ItemDownshelf", x => InvokeEvent(ItemDownshelf, x));

            //商品上架消息
            HubProxy.On<object>("ItemUpshelf", x => InvokeEvent(ItemUpshelf, x));

            //商品新增消息
            HubProxy.On<object>("ItemAdd", x => InvokeEvent(ItemAdd, x));
        }
