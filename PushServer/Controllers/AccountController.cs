...
namespace PushServer.Controllers
{
    ...
    public class AccountController : Controller
    {
        public ApplicationDbContext DbContext
        {
            get
            {
                return this.HttpContext.GetOwinContext().Get<ApplicationDbContext>();
            }
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.UserName, Email = model.UserName + "@sandsea.info", CreateDate = DateTime.Now };
        ...
        public ActionResult AccountList(string kw)
        {
            var ctx = this.HttpContext.GetOwinContext().Get<ApplicationDbContext>();
            var list = ctx.Users.Include(x => x.Connections).Where(x => !string.IsNullOrEmpty(kw) ? x.UserName.Contains(kw) : true).OrderByDescending(x => x.CreateDate).ToList();
            return View(list);
        }

        public ActionResult Heartbeat(string id)
        {
            SignalRServer.Instance.Clients.Client(id).OnMessage("Server Heartbeat " + DateTime.Now);
            return JavaScript("alert('测试心跳已发送。')");
        }
